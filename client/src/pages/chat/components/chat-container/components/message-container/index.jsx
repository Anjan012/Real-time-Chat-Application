...
const MessageContainer = () => {
  ...
  const { selectedChatType, selectedChatData, selectedChatMessages, setSelectedChatMessages, setFileDownloadProgress, setIsDownloading, userInfo } = useAppStore();
  ...
  const downloadFile = async (url) => {
    ...
    }); // fetching the file using axios

    const urlBlob = window.URL.createObjectURL(new Blob([response.data])); // creating a object URL
  ...
  const renderDMMessages = (message) => (
    <div className={`${message.sender === selectedChatData._id ? "text-left" : "text-right"}`}>
      ...
      }
      {
        ...
        <div className={`${message.sender !== selectedChatData._id ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50 " : "bg-[#2a2b33]/5 text-white/80 border-white/20"} border inline-block p-4 rou my-1 max-w[50%] break-words`}>
          {checkIfImage(message.file) ? <div className="cursor-pointer" onClick={() => { setShowImage(true); setImageUrl(message.file) }}> <img src={`${HOST}/${message.file}`} height={300} width={300} /> </div> : (
            <div className="flex items-center justify-center gap-4">
              ...
              <span className="bg-black/20 p-3 text-2xl rounded-full hover:bg-black/50 cursor-pointer transition-all duration-300" onClick={() => downloadFile(message.file)}>
                <IoMdArrowRoundDown />
              </span>
            </div>
          )}
  ...
  const renderChannelMessages = (message) => {
    return(
      <div className={`mt-5 ${message.sender._id !== userInfo.id ? "text-left" : "text-right"}`}>
        {
        message.messageType === 'text' && (
          <div className={`${message.sender._id === userInfo.id ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50 " : "bg-[#2a2b33]/5 text-white/80 border-white/20"} border inline-block p-4 rou my-1 max-w[50%] break-words ml-9`}>
            {message.content}
          </div>
        )
      }
      ...
      {
        message.messageType === "file" &&
        <div className={`${message.sender._id === userInfo.id ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50 " : "bg-[#2a2b33]/5 text-white/80 border-white/20"} border inline-block p-4 rou my-1 max-w[50%] break-words`}>
          {checkIfImage(message.file) ? <div className="cursor-pointer" onClick={() => { setShowImage(true); setImageUrl(message.file) }}> <img src={`${HOST}/${message.file}`} height={300} width={300} /> </div> : (
            <div className="flex items-center justify-center gap-4">
              <span className="text-white/8- text-3xl bg-black/20 rounded-full p-3">
                <MdFolderZip />
